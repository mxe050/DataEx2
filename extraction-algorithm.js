...
4. 二値変数（Binary）:
   ...
   - percent のみの場合は実数を逆算し notes に記載（例: "5 (10.4%)" → 分母候補 48: 5/48=10.4%）
   - ゼロイベントは必ず "0" として記録（空欄禁止）。
     - **両群とも 0 イベント**の場合: events_or_mean=0 を両群に記録した上で、notes に "[両群0イベント: アウトカム評価実施済み]" と明記（"報告なし" との区別が重要 — メタ分析の double-zero 扱いに必要）
