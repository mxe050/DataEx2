...
【📜 一般ルール（最優先・全タスク共通の暗黙前提）】
1. **根拠は提供PDFのみ**: 提供されたPDF本文・画像・表だけを根拠にする。外部知識・先行研究・推測は使わない
2. **値の捏造禁止**: 推測・補間・"妥当な推定値"は禁止。報告されていなければ "不明" と記録する（"not reported" の意）
3. **表・図にしか値が無い場合も読み取る**: 本文に書かれていなくても、表（Table）や図（Figure / KMカーブ / 棒グラフ）に値があれば必ず抽出する
4. **明示値からの単純算術は許可**: per-patient テーブル行のカウント / 行合計 / n/total×100 のパーセント計算は OK。ただし notes に "[計算: <式>]" を記録
5. **複数値の網羅保持**: 同一変数で複数の指標（mean と median 両方 / 異なる time point / 異なる解析集団）が記載されていれば、**全て**を別レコードとして保持する。情報損失（片方だけ採用、統合）は禁止
...
【🧬 群名(arm)の一貫性 — 表ごとに表記が揺れても同一群とみなす】
論文内で同じ群が以下のように表記されることがある:
- 介入群: "Intervention" / "Treatment" / "Experimental" / "Test" / "Active" / "Drug A" / "Group 1" / 商品名・略号（例: PCR, SGLT2i）
- 対照群: "Control" / "Placebo" / "Standard care" / "Usual care" / "Sham" / "Group 2" / 略号（例: SW）
最初に Methods を読んで canonical な介入群/対照群名を確定してから Results / Tables / Figures を読む。
表ごとにラベルが変わっても **N、ベースライン特性、用量、ランダム化日** が一致すれば同一群として処理。
ラベル揺れがある場合は notes に "[群名揺れ: Methods=Drug A / Table 2=Group 1]" を記録し、取り違えがないことを明示。
特に「Group 1 / Group 2」のような中立ラベルは Methods 定義を必ず参照（番号順 = 介入順とは限らない）。
...
2. カンマ/ピリオド/スペースを数値区切りとして誤解釈しない（例: "1,234" = 1234, "1.234" = 小数, 言語圏に注意）
   - 欧州式表記の論文では "3,5" が "3.5" の意味（カンマが小数点）。Methods/Tables の脚注で表記方式を必ず確認
   - 千位区切り: 英米式 "1,234" = 1234 / 欧州式 "1.234" = 1.234（小数）— 文脈で判別
2-OCR. **OCR/PDF テキスト化時の典型的な誤読**を疑う:
   - 数字: 0↔O, 1↔l↔I, 5↔S, 8↔B（特に表セル内）
   - 信頼区間記号: "95% Cl" は "95% CI" の OCR ミスの可能性大
   - 不等号: "≤" "≥" が "<=" ">=" / "<" ">" として読まれている可能性
   - 誤差記号: "±" が "+", "+/-", "+- " と表記されている可能性
   - ハイフン類: 範囲表記の "0.23–0.63" の「–」が "-" / "—" に置き換わっていることがある
   - 不自然な値（例: VAS 0-10 で mean=78、p値 = 5.0 等）は単位誤認/桁誤読を疑い、notes に "[要確認: OCR誤読/単位/桁の可能性]" を記録
2-FMT. **出力フォーマットの統一（メタ分析ソフトに直結する形に揃える）**:
   - 二値変数の events_or_mean: "events (percent%)" 形式（例: "91 (80%)"）。"/" 区切りで n_total を含めない（n_total は別フィールド）
   - 連続変数の events_or_mean: 中心値のみ（例: "32"）。SD/IQR/range は sd_or_se に分離
   - sd_or_se の表記は variance 種別を明示:
     - "0.8" のような数値のみ → SD と仮定（sdOrSe="SD"）
     - "(0.8)" / "± 0.8" → 文脈から SD or SE を判定
     - "0.5–1.2" / "(0.5-1.2)" → IQR or range（中央値とセットで報告される）。sdOrSe に "IQR" or "range" を必ず明記
   - 単位: events_or_mean に単位を含めない（"5 mg" でなく "5"）。単位は actualOutcomeName または notes に記録
   - **単位欠落の検出**: 連続変数で単位が論文中に書かれていなければ notes に "[単位欠落 — 要確認]" を記録
   - p値: 原文表記をそのまま保持（"<0.001" / "0.026" / "NS"）。数値変換しない
   - 95%CI: "lower–upper" / "lower to upper" / "lower, upper" の何れの原文表記でもよいが、ci95 フィールドには両端を含む文字列を入れる
   - pre/post 対比: 同一アウトカムで pre と post の両方が報告されていれば、別レコードとして両方保持し、actualOutcomeName に "[pre]" / "[post]" / "[change post-pre]" を必ず明記
...
4. 二値変数（Binary）— **events_or_mean には必ずイベント数（整数）を記録、% 単独を入れることは禁止**:
   ⚠ メタ分析ソフト（RevMan / R metafor）は raw events / total を要求する。% 値をそのまま events_or_mean に格納する誤りが頻発するため、以下を厳守:

   優先順位 (上から順に試す):
   (a) **原文に実数 n (events/total) が明示**: 最優先で抽出（例: "91/114 (80%)" → events_or_mean="91 (80%)"、"80% (91/114)" でも同じ）
   (b) **% のみ報告 + 分母 N が確定**: events = round(N × percent / 100) を計算して整数で格納
       - 例: "Success rate 80% in PCR group" + N=114 → events_or_mean="91 (80%)"（91 = round(114 × 0.80)）
       - 例: "26% complications" + N=50 → events_or_mean="13 (26%)"（13 = round(50 × 0.26)）
       - 計算した場合は notes に "[計算: events = N × percent = <N> × <p%>/100 = <events>]" を必ず記録
       - 計算で復元した値は confidence を **"medium" に下げる**（原文に整数記載でないため）
   (c) **% のみ + 分母 N が不明**: その時に限り events_or_mean に "80%" のように % 表記を残し、notes に "[%のみ：分母不明 — 著者照会候補]" を記録
   (d) **数値文字列だけが書いてあり単位不明**: 推測しない。"不明" と記録

   🚫 **絶対禁止パターン**:
   - 原文 "80%" → events_or_mean="80" （← % を整数として誤記録: メタ分析で 80/114 = 70% という誤った率になる）
   - 原文 "80%" → events_or_mean="80%" でかつ N が判明している （← 分母があるなら計算すべき）

   その他のルール:
   - ゼロイベントは必ず "0" として記録（空欄禁止）。
     - **両群とも 0 イベント**: events_or_mean=0 を両群に記録した上で notes に "[両群0イベント: アウトカム評価実施済み]" と明記
     - **片群のみ 0**: notes に "[片群0イベント — sparse data]" を記録
...
- "[多群: 対照群分割]"
- "[多群: 別比較として独立]"
本アプリは2群比較に絞って抽出するが、**論文が3群以上の場合は最も関連する2群比較**（PICOに照らして主比較）を選択し、
- 選択した比較ペア（例: "比較選択: Drug A 対 Placebo（Drug B は除外）"）を notes に記録
- 同一対照群を複数比較で使う場合は "[警告: 共通対照群 — 二重カウントリスク]" を notes に記録

■ 特殊デザインを明示
以下のいずれかに該当する場合、並行群RCTとして扱うと単位解析(unit-of-analysis)エラーが発生するため、必ず notes に明記:
- crossover / cluster RCT / split-mouth / paired-eye / bilateral body-part / matched-pair / factorial / stepped-wedge / non-inferiority
notes 記録例:
- "[特殊デザイン: cluster RCT, ランダム化単位=clinic, 解析単位=patient → ICC 確認要]"
- "[特殊デザイン: split-mouth, ランダム化単位=tooth site, 同一被験者内ペア → paired解析必要]"
- "[特殊デザイン: crossover, washout 期間=2週, period effect 確認要]"
ランダム化単位（patient / cluster / tooth / eye / lesion / cycle）と解析単位が不一致なら **必ず両方を notes に記録**。
分母単位を取り違えると n_total が誤って大きくなる（例: 20人を tooth 単位で 80 と書く）ため、Methods の "Unit of randomization / Unit of analysis" 記述を最優先で確認。
...
11. **数学的整合性**: events_or_mean ≤ n_total / SD・SE が負でない / VAS スケール上限超過(0-10で>10、0-100で>100) / CI lower < value < upper / p値が 0-1 の範囲内
12. **効果指標の方向と CI/p の整合性**: HR/RR/OR < 1 で CI lower > 1 のような矛盾 / 95%CI が 1.0 を跨がないのに p > 0.05 のような矛盾 / log scale 値の符号反転忘れ
13. **群名(arm)の取り違え**: Methods で定義された介入群と Results/Tables で抽出された介入群が同一か（"Group 1/Group 2" のような中立ラベルは特に要確認）
14. **ゼロイベント区別**: events=0 と "報告なし"(空欄)の区別が notes で明示されているか（[両群0イベント: アウトカム評価実施済み] 等）
15. **特殊デザインの単位解析**: cluster/split-mouth/crossover 等の場合、ランダム化単位と解析単位の不一致が notes に記録されているか
16. **partial 検出（分散値欠落）**: 連続変数で mean / median は記録されているが SD / IQR / range が空 or "-" → メタ分析の pooling 不可になるため致命的。notes に "[partial: 分散指標欠落 — メタ分析 pooling 不可、著者照会候補]" を必ず記録
17. **単位欠落検出**: 連続変数で値があるのに単位（months / years / mm / mg / kg 等）が actualOutcomeName / notes のどこにも書かれていなければ notes に "[単位欠落 — 要確認]" を追記
18. **complete failure と partial の区別**: notes に以下のいずれかを明示
    - "[complete failure: 全数値フィールド抽出不能 — 論文に記載なし or 抽出失敗]"
    - "[partial: <欠落項目を列挙>]"
    これにより「アウトカム評価実施 + 報告なし」と「単純な抽出失敗」が区別できる
19. **複数値の網羅性チェック**: 同一変数で複数の指標（mean と median 両方 / 異なる time point / 異なる解析集団）が原論文に記載されているのに片方しか抽出されていない場合、欠落分も新規 outcome として correctedOutcomes に追加（情報損失の防止）
20. **出力フォーマットの統一性**: events_or_mean に "/" や "n_total" が混入していないか、sd_or_se と effectEstimate.value が混同されていないか
21. **🚨 % を events として誤記録していないか（最頻バグ）**: 二値アウトカムで、evidenceText に "X%" の形式しか書かれていないのに events_or_mean に X がそのまま入っている場合、それは **% を整数イベント数として誤記録**している。N が判明していれば events = round(N × percent / 100) で再計算して **整数** に修正し、notes に "[Critic修正: % をイベント数として誤記録していたため events = N × p%/100 で再計算]" を記録。
    検出ヒューリスティック:
    - evidenceText に "<数値>%" があり、その数値が events_or_mean とほぼ同一値
    - かつ evidenceText に "X/Y" のような分数表記が無い
    - かつ events_or_mean が分母 N より小さい（当然だが、% 値もそうなる）
    - 上記が揃えば、% 誤記録の可能性が極めて高い → 必ず再計算して修正
...
9. **数学的整合性**: 各 outcome について以下を検算し、矛盾あれば修正 + notes に "[Critic数値検算: <内容>]"
   - events_or_mean ≤ n_total（events が n を超えていないか）
   - SD・SE は非負、不自然に大きすぎないか（SE > SD のケースは稀）
   - VAS 0-10 で mean > 10 / VAS 0-100 で mean > 100 などスケール上限超過
   - 95%CI の lower < value < upper の順序
   - p値が 0-1 の範囲内
10. **効果指標の方向と CI/p の整合性**:
    - HR/RR/OR < 1 で 95%CI lower > 1 のような明らかな矛盾
    - 95%CI が 1.0 を跨がないのに p > 0.05 のような不一致
    - HR の reference group の向き（intervention vs control が標準）。逆向きで反転していない場合は反転 + "[HR反転: 元 control vs intervention]"
11. **群名(arm)の取り違え**: Methods で定義された介入群と Tables で抽出された介入群が同一か。"Group 1/Group 2" のような中立ラベルは Methods 定義を必ず参照
12. **ゼロイベント区別**: events=0 と "報告なし"(空欄)の区別が notes に明記されているか（[両群0イベント: アウトカム評価実施済み] 等）
13. **特殊デザイン**: cluster/split-mouth/crossover/factorial/stepped-wedge のいずれか該当なら、ランダム化単位と解析単位の不一致が notes に記録されているか
14. **OCR 誤読の疑義**: VAS 0-10 で mean=78、p=5.0 など不自然な値、"95% Cl" 表記、"±" の "+/-" 化など、OCR 誤読の痕跡があれば notes に "[要確認: OCR誤読疑義]"
15. **partial 検出（分散値欠落）**: 連続変数で mean / median はあるが SD / IQR / range が空・"-"・"不明" → メタ分析 pooling 不可。notes に "[partial: 分散指標欠落 — pooling 不可、著者照会候補]" を必ず追記
16. **単位欠落検出**: 連続変数で値がありながら単位（months / years / mm / mg / kg / VAS 等）が actualOutcomeName / notes に書かれていなければ notes に "[単位欠落 — 要確認]"
17. **complete failure vs partial の区別**: notes に以下のどちらかを明示 — "[complete failure: 全数値フィールド抽出不能]" or "[partial: <欠落項目>]"
18. **複数値の網羅性**: 原論文に同一変数の複数指標（mean と median 両方 / 複数時点 / ITT と PP 両方）が記載されているのに片方しか抽出されていなければ、欠落分を correctedOutcomes に新規追加（情報損失禁止）
19. **フォーマット統一**: events_or_mean に "n/total" や "/" が混入していないか / sd_or_se と effectEstimate.value が取り違えられていないか / 単位が events_or_mean セルに混入（"5 mg" など）していないか
20. **値の捏造禁止チェック**: evidenceText に書かれていない値が構造化フィールドに入っていないか（捏造・推測値の検出 — 該当時は notes に "[捏造疑義: <フィールド名> evidenceText 未記載]" を記録の上、該当値を "不明" に戻す）
21. **🚨 % を events として誤記録の検出と修正（最頻バグ・必ずチェック）**:
    二値アウトカムで evidenceText に "X%" 表記しか無いのに events_or_mean に X がそのまま整数として記録されているケースを検出。
    例: evidenceText="success rates of 80% in PCR group", interventionGroup.n_total="114", interventionGroup.events_or_mean="80"
    → これは "80%" を整数イベント数として誤記録。真の events = round(114 × 0.80) = 91。events_or_mean を "91 (80%)" に修正。

    検出条件 (ALL満たす場合):
    (a) evidenceText に "<数値>%" 形式の記載がある
    (b) その数値 P が events_or_mean とほぼ一致（例: P=80, events_or_mean="80"）
    (c) evidenceText に "X/Y" や "X out of Y" のような分数表記が無い（実数記載がない証拠）
    (d) n_total が確定している

    修正手順:
    - events = round(n_total × P / 100) を計算
    - events_or_mean を "<events> (<P>%)" 形式に上書き（例: "91 (80%)"）
    - notes に "[Critic修正: %値をイベント数として誤記録 → events = N × p%/100 = <N> × <P>/100 = <events>]" を追記
    - confidence を "medium" に下げる（原文に整数記載がない計算復元のため）

    ⚠ 例外: evidenceText に "91 (80%)" のように整数+%が併記なら events_or_mean="91 (80%)" は正しい。修正不要。
...
Step 6 (二値専用・% 誤記録防止): 抽出値が "X%" のような % 表記しかない場合、整数イベント数として記録するのは厳禁。
   - N が判明していれば events = round(N × p%/100) を計算（例: N=115, p=80% → events=92）
   - events_or_mean には "<events> (<p>%)" 形式で格納（例: "92 (80%)"）
   - notes に "[計算: events = N × p% = 115 × 80/100 = 92]" を必ず記録
   - N が不明な場合に限り events_or_mean に "80%" を残し、notes に "[%のみ：分母不明]"
...
- フィールドを空のままにしない
- 🚨 **% 値をイベント数として記録するな**: 原文に "56%" のような % のみで実数イベント数が無い場合、N が判明していれば events = round(N × p%/100) で計算して整数で格納（例: N=114, p=56% → events="64 (56%)"）。% 値そのものを events_or_mean に入れることは禁止
